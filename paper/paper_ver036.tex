...
The campaign above is bounded to the open-space grid (random obstacle
topology, Section~VIII). After submission we ran a post-submission topology
dossier to map the boundary of the confirmed effects, keeping the manuscript
frozen: the numbers and narrative below are confirmatory extensions, not part
of the preregistered protocol. Two new environment families extend
...
comparisons on \texttt{quality\_auc} @ $T$ (primary) with the final-coverage
guard, and budget $T = 0.7 \times$ FB median \texttt{steps\_90} measured per
topology by a probe (maze 1323, cluster A6 1354, cluster A3 2710 steps).
...
\begin{table}[t]
\caption{Maze family, \texttt{quality\_auc} @ $T$ (higher better), $n = 40$ paired per campaign; rel-gain\% vs baseline B per the paper's convention; Holm across the 3 pairs. Perfect maze (A6): both candidates lose significantly. H=16: CU is bit-identical to H=8 ($-2.1\%$, Holm-sig), RA improves to ns.}
\label{tab:topo_maze}
\centering
...
\caption{Sixteen Kruskal perfect mazes (topology=maze, $100 \times 100$,
...
\begin{table}[t]
\caption{Cluster family, \texttt{quality\_auc} @ $T$ (higher better), $n = 40$ paired per campaign. A6 (T=1354): RA +3.0\% vs FB (Holm-sig), RA $\geq$ CU confirmed, CU neutral. A3 pass-2 (T=2710): RA +2.6\% vs FB (Holm-sig), RA vs CU +6.3\% (strongest delta), CU neutral.}
...
\caption{Sixteen cluster maps (topology=cluster, $100 \times 100$, 20\%
...
What matters is routing redundancy, not spatial fragmentation per se. This is a
post-submission confirmatory extension and is labeled as such; the paper's
headline claims (sparse open space, Sections~VI-D--VI-J) are unchanged.
...
variable.}
The post-submission topology dossier (Section~VI-K) isolates the mechanism.
...
the topology the effect is claimed for. A post-submission topology dossier
...
globally. The post-submission topology dossier (Section~VI-K) sharpens the
