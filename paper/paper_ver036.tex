...
config-count proxy captures. Section~VI-I reports the completed campaign, its
...
\begin{table*}[t]
\caption{Maze family, \texttt{quality\_auc} at mission end (budget $T$) (higher better), $n = 40$ paired per campaign; rel-gain\% vs baseline B per the paper's convention; Holm across the 3 pairs. Perfect maze (A6): both candidates lose significantly. H=16: CU is bit-identical to H=8 ($-2.1\%$, Holm-sig), RA improves to ns.}
\label{tab:topo_maze}
\centering
\small
...
space, Sections~VI-D--VI-J) are unchanged.
...
variable.}
The topology dossier (Section~VI-K) isolates the mechanism.
...
(Section~VI-K) extends this with maze and cluster layouts under the same LOS
...
(iii) the maze/cluster dossier (Section~VI-K) shows the advantage survives
...
the dead-end misjudgment directly; (iv) adaptive $\lambda$
...
blocked fraction (e.g.\ $\fcnt / \farea < 0.5$,
...
globally. The topology dossier (Section~VI-K) sharpens the
...
claims of the paper (sparse open space, Sections~VI-D--VI-J) are unchanged.
