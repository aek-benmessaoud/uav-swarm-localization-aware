"""
analysis/ra_sweep_stats.py — RA (Richness-Angular) through the sigma-loc sweep.

The paper's sweep B (Table 12) reports only Coverage-U vs Frontier-Bounded at
sigma_loc in {0.5, 1.0}. RA was ALREADY run in the same campaign (paired
runs/seeds) but never analyzed. RA is now the accuracy-optimal member of the
config-count family (MH #2/MH #3), so its robustness under self-localization
noise is a reporting gap, not a new experiment.

Protocol identical to sweep_stats.py: paired Wilcoxon (zero_method="wilcox"),
Holm-Bonferroni across the 2 regimes per variant family, delta = m/n^2,
rel-red by median relative change (lower_better).

Reports, at each sigma level, per regime:
  - RA vs FB  (does RA's edge over the movement frame survive noise?)
  - RA vs CU  (does RA's advantage over Coverage-U survive noise?)
plus the coverage guard (RA vs FB) and quality_auc context.

Usage: python analysis/ra_sweep_stats.py
"""
...
def rel_gain(va, vb, lower_better):
    # Matches the paper's convention exactly (paper_build.py: element-wise
    # 100*(vb - va)/vb then np.nanmedian), baseline (vb) always the denominator.
...
def main():
    print("# RA (Richness-Angular) through the sigma-loc sweep "
          "(mean_bound_final @ T, lower better)\n")
    for variant in VARIANTS:
        print(f"{'=' * 78}\n## sigma_loc level = {variant} "
              f"({'0.5' if variant == 's05' else '1.0'} cells)\n{'=' * 78}")
